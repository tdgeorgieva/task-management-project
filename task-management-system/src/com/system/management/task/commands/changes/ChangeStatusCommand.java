...
import static com.system.management.task.commands.CommandConstants.CHANGE_STATUS;
...
public class ChangeStatusCommand extends BaseCommand {
    ...
    public ChangeStatusCommand(RepositoryHelper repoHelper) {
        super(repoHelper);
    }
    ...
    protected String executeCommand(List<String> parameters) {
        ...
        Status status = ParsingHelpers.tryParseEnum(parameters.get(1).toLowerCase(), Status.class);
        changeStatus(id, status);
        return CHANGE_STATUS;
    }

    private void changeStatus(int id, Status status) {
        getRepoHelper().findElementById(id, getRepoHelper().getItems()).changeStatus(status);
    }
}
