...
public class ChangeBugPriorityCommand_Tests {
    ...
    void init() {
        ...
        command = new ChangeBugPriorityCommand(repoHelper);
    }

    @Test
    public void changeStoryPriority_Should_ChangePriority_When_ValidParameters() {
        command.execute(Arrays.asList("1", "Low"));
        Assertions.assertEquals(repoHelper.getBugs().get(0).getPriority().toString(), "Low");
    }
