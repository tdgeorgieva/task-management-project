...
import com.system.management.task.commands.changes.ChangeRatingCommand;
...
public class ChangeRatingCommand_Tests {
    ...
    void init() {
        ...
        repoHelper.createFeedback(VALID_TEAM_NAME,VALID_BOARD_NAME,VALID_TITLE, VALID_DESCRIPTION, Status.DONE, 2 );
        command = new ChangeRatingCommand(repoHelper);
    }

    @Test
    public void changeFeedbackRating_Should_ChangeRating_When_ValidParameters() {
        command.execute(Arrays.asList("1", "3"));
        Assertions.assertEquals(repoHelper.getFeedbacks().get(0).getRating(), 3);
    }
