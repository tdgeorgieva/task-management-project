...
import com.system.management.task.commands.changes.ChangeStorySizeCommand;
...
public class ChangeStorySizeCommand_Tests {
    ...
    void init() {
        ...
        command = new ChangeStorySizeCommand(repoHelper);
    }

    @Test
    public void addComment_Should_AddComment_When_ValidParameters() {
        command.execute(Arrays.asList("1", "Small"));
        Assertions.assertEquals(repoHelper.getStories().get(0).getSize().toString(), "Small");
    }
