...
public class RemoveCommentCommand_Tests {
    ...
    void init() {
        ...
        command =  new RemoveCommentCommand(repoHelper);
    }
    @Test
    public void addComment_Should_RemoveComment_When_ValidParameters() {
        command.execute(Arrays.asList( "content","1","author"));
        Assertions.assertNotNull(repoHelper.findWorkItemById(1).getComments().contains(new CommentImpl("author", "content")));
    }
    @Test
